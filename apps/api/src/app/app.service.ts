    ...
    return this.http
      ...
      .pipe(
        switchMap((r) => {
          ...
          const audios = epsUrls.map((u) => {
            return this.http.get(`https://www.rtp.pt${u}`, { headers }).pipe(
              map((r) => {
                const $: CheerioAPI = load(r.data);
                return {
                  url: `https${
                    r.data.split('var f = "https')[1].split('.mp3')[0]
                  }.mp3`,
                  label: $('meta[itemprop="name"]').attr('content'),
                  thumb: r.data.split('poster: "')[1].split('",')[0],
                  duration: $('.input-group-addon.addon-result').text(),
                };
              })
            );
          });
